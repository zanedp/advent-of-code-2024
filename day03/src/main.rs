...
fn main() {
    ...
    let mul_sum_pt1 = get_pairs(input).fold(0, |acc, (x, y)| x * y + acc);
    dbg!(mul_sum_pt1);
    ...
    let mul_sum_pt2 = get_pairs(&recombined).fold(0, |acc, (x, y)| x * y + acc);
    dbg!(start_time.elapsed());
    dbg!(mul_sum_pt2);
}

fn get_pairs(input: &str) -> impl Iterator<Item = (u64, u64)> + use<'_> {
    static RE: LazyLock<Regex> =
        LazyLock::new(|| Regex::new(r"mul\((?<a>\d+),(?<b>\d+)\)").unwrap());
